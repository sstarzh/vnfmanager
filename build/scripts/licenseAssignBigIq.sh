...
# install a license to BIG-IP using BIG-IQ (unmanaged pool, reachable device)
...
DATA='{ "licensePoolName": "'"$license_pool"'", "command": "assign", "address": "'"$bigip_host"'", "user": "'"$user"'", "password": "'"$passwd"'" }'
...
if [[ $response_code != 202 ]]; then
     echo "Failed to begin license assigment; exiting with response code '"$response_code"'"
     exit 1
fi